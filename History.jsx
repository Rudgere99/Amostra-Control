  ...
  async function updateCollection(updatedRow) {
    ...
    try {
      let savedRow = {
        ...
        letter: updatedRow.letter || ''
      }

      if (hasApiConfigured()) {
        const rowId = String(updatedRow.id || '')
        const canUpdateRemote = Boolean(updatedRow.remote && rowId && !rowId.startsWith('novo-') && !rowId.startsWith('programacao-'))

        if (canUpdateRemote) {
          try {
            savedRow = await updateCollectionApi(updatedRow.id, savedRow)
          } catch (error) {
            if (!String(error.message || '').includes('Coleta não encontrada')) {
              throw error
            }

            savedRow = await createCollection({
              ...savedRow,
              id: undefined,
              remote: undefined
            })
        ...
        await loadDashboardStats({ date: savedRow.date, plant: savedRow.plant })
      }

      setSchedule((current) => {
        const sameSlot = (row) => row.date === updatedRow.date && row.plant === updatedRow.plant && String(row.time).slice(0, 2) === String(updatedRow.time).slice(0, 2)
        const exists = current.some((row) => row.id === updatedRow.id || sameSlot(row))
        if (!exists) return [savedRow, ...current]
        return current.map((row) => (row.id === updatedRow.id || sameSlot(row)) ? savedRow : row)
      })
